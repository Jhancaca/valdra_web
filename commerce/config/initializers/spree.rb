...
Rails.application.config.after_initialize do
  if ENV["SUPABASE_PRIVATE_BUCKET"].present?
    Spree.private_storage_service_name = :supabase_private
    Spree.public_storage_service_name = :supabase
  end

  # Spree 5.6 still emits deprecation warnings through its own
  # ActiveSupport::Deprecation instance. The default behavior writes directly
  # to `$stderr`; when Puma is started from a terminal that stream can be
  # closed while a request is rendering, which raises Errno::EPIPE and masks
  # the actual admin response. Keep the warnings in Rails' log instead.
  Spree::Deprecation.behavior = :log
